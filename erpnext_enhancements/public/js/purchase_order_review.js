...
function collectUpdatedData() {
    const updatedData = {
        hasChanges: false,
        custom_remarks: ''  // Change from 'remarks' to 'custom_remarks'
    };

    // Collect custom_remarks only
    const remarksTextarea = document.getElementById('po-remarks');
    if (remarksTextarea && remarksTextarea.value.trim()) {
        updatedData.custom_remarks = remarksTextarea.value.trim();  // Change field name
...
function saveUpdatedData(frm, updatedData) {
    return new Promise((resolve, reject) => {
        // Update custom_remarks in the current form
        if (updatedData.custom_remarks) {
            frm.set_value('custom_remarks', updatedData.custom_remarks);  // Change field name
        }
