...
def debounced_item_search(doctype, txt, searchfield, start, page_len, filters):
    ...
    if search_duration > 0.05:
        frappe.log_error(
            f"Item search took {search_duration:.3f}s for query '{txt}' - Priority: exact custom_sku_code > exact item_code > exact item_name > partial item_code > partial item_name > partial custom_sku_code",
